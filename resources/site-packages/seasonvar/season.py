...
class Season:
    ...
    def playlists(self):
        if self._playlists is None:
            self._playlists = self._find_playlists()
    ...
    def _playlist4translation(self, translation):
        ...
        return re.sub(r'trans[^/]+/', r'trans/',
                      self.playlists[0]['playlist_url'])
    ...
    def _find_playlists(self):
        ...
        if tb is None:
            return [x for x in self._unnamed_playlists(self.__html)]
        else:
            return [x for x in self._named_playlists(tb)]

    @staticmethod
    def _named_playlists(htmlblock):
        r = re.compile(
    ...
    @staticmethod
    def _unnamed_playlists(htmlblock):
        r = re.compile(r'var\s+pl\d+\s+=\s+"(.*?)";')
        for url in r.findall(htmlblock):
            yield {'translation': 'Unknown', 'playlist_url': url.strip()}
