...
try:  # python2
    ...
    from urlparse import parse_qs as original_parse_qs
    ...
    def parse_qs(obj):
        d = original_parse_qs(obj)
        ret = {}
        for k, v in d.items():
            utf8k = unicode(k, 'utf-8')
            ret[utf8k] = []
            for i in v:
                ret[utf8k] += [unicode(i, 'utf-8')]
        return ret
...
class __logger:
    ...
    def error(self, s):
        return xbmc.log(s, xbmc.LOGERROR)
...
def show_notification(title, message, icon, timeout=2000,
                      proportionalTextLengthTimeout=False):
    if proportionalTextLengthTimeout:
