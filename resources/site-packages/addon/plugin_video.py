...
def screen_episodes(args):
    url = args.get('url')
    if url is None:
        logger.error('screen_episodes: "url" missing {0}'.format(args))
        return
    tr = args.get('tr')
    ...
    if seasons_count > 1:
        add_directory_to_list(
            '[COLOR FFFFD700]сезон[/COLOR]: '
            '{0} / {1}'.format(season.number, seasons_count),
            addon.make_url({'action': 'screen_seasons', 'url': url}),
            items_count=len(series.seasons)
        )

    if len(season.playlists) > 1:
        add_directory_to_list(
            '[COLOR FFFFD700]озвучка[/COLOR]: '
            '{0}'.format(tr if tr is not None else 'Стандартная'),
            addon.make_url({'action': 'screen_translations', 'url': url}),
            items_count=len(season.playlists)
        )

    for episode in season.episodes(tr):
        add_item_to_list(
            episode['name'],
...
def screen_translations(args):
    url = args.get('url')
    if url is None:
        logger.error('screen_translations: "url" missing {0}'.format(args))
        return
    tr = args.get('tr')

    show = Show(url)
    season = show.current_season
    for n, pl in enumerate(season.playlists):
        if tr is None and n == 0 or pl['translation'] == tr:
            prefix = '* '
        else:
            prefix = ''
        add_directory_to_list(
            '{0}{1}'.format(prefix, pl['translation']),
            addon.make_url({
                'action': 'screen_episodes',
                'url': season.url,
                'tr': pl['translation'],
            }),
            thumb=season.thumb
...
def screen_seasons(args):
    url = args.get('url')
    if url is None:
        logger.error('screen_seasons: "url" missing {0}'.format(args))
        return
    ...
    for season in series.seasons:
        if season.number == series.current_season.number:
            ...
            prefix = ''
        add_directory_to_list(
            '{0}сезон {1}'.format(prefix, season.number),
...
def play(args):
    url = args.get('url')
    if url is None:
        logger.error('play: "url" missing {0}'.format(args))
        return
...
def get_user_input(heading):
    ...
    if (keyboard.isConfirmed()):
        # getText() return string and not unicode
        # so convert to unicode here
        return unicode_text(keyboard.getText())
    else:
        # because of unicode_literals unicode will be returned
        return ''
...
def main():
    logger.info("addon args:{0}".format(addon.args))
