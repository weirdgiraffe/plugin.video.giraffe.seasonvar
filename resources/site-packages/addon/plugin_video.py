...
def screen_date(args):
    ...
    for item in date_items:
        ...
        add_directory_to_list(
            displayed_name,
            addon.make_url({'action': 'screen_episodes', 'url': item['url'], 'name': item['name'].encode('utf-8')}),
            thumb=s.thumb)
    list_end()
...
def screen_episodes(args):
    ...
    if seasons_count > 1:
        add_directory_to_list(
            ...
            '[COLOR FF0FF000] Сезон [/COLOR] {sn} / {sc}'.format(serial=name, sn=season.number, sc=seasons_count),
